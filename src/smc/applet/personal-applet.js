...
class PersonalApplet {
  ...
  async getInfo(query = ['cid']) {
    ...
    if (q.cid) {
      ...
      // console.log(data.slice(0, -2).toString(),'444444444444');
    }

    // if (q.laser) {
    //   data = await reader.getData(this.card, apduPerson.CMD_LASER, this.req);
    //   info.laser = data.slice(0, -2).toString();
    //   console.log(data.toString(),'555555555555555555');
    // }
    ...
    if (q.address) {
      ...
      info.address = {
        ...
        street: data
          ...
          .join(' ')
          .trim(),
        subdistrict: data[data.length - 3].replace('แขวง', '').replace('ตำบล', '').trim(),
        district: data[data.length - 2].replace('เขต', '').replace('อำเภอ', '').trim(),
        province: data[data.length - 1].replace('จังหวัด', '').trim(),
